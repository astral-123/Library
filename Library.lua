...
-- AMÉLIOREZ aussi la section Settings pour refresh le dropdown après save/delete :

-- Dans le Settings Tab, MODIFIEZ la section Config System comme ceci :

local SettingsRight = Tab:AddSection("Config System", "right")

local configNameInput = ""
local selectedConfig = "None"
local configDropdown -- On va stocker la référence
    ...
    Callback = function()
        if configNameInput ~= "" then
            ...
            if success then
                -- Refresh le dropdown
                task.wait(0.1)
                local newConfigs = Window:GetConfigList()
                -- Vous devrez implémenter une méthode pour update le dropdown
                -- ou recréer la section Settings
            end
        else
            Window:Notification({
                Title = "Error",
    ...
    Callback = function()
        if selectedConfig ~= "None" then
            if delfile then
                ...
                Window:Notification({
                    Title = "Deleted",
                    Text = "Config '" .. selectedConfig .. "' deleted!",
                    Duration = 3
                })
                selectedConfig = "None"
                -- Refresh le dropdown
...
SettingsRight:AddButton({
    ...
    Callback = function()
        ...
        -- Pour l'instant, l'utilisateur devra reopen le UI
        -- Une vraie solution nécessiterait de reconstruire le dropdown
    end
})
